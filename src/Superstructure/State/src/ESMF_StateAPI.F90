...
!------------------------------------------------------------------------------ 
 ...
 linkChange = ESMF_TRUE 
 call c_ESMC_AttributeLink(state%statep%base, & 
 routehandle, linkChange, localrc) 
 if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
...
!------------------------------------------------------------------------------ 
 ...
 linkChange = ESMF_TRUE 
 call c_ESMC_AttributeLink(state%statep%base, & 
 nestedState%statep%base, linkChange, localrc) 
 if (ESMF_LogFoundError(localrc, ESMF_ERR_PASSTHRU, & 
 ESMF_CONTEXT, rcToReturn=rc)) return 
...
!
! !INTERFACE:
      subroutine ESMF_StatePrint(state, options, nestedFlag, filename, rc)
!
! !ARGUMENTS:
      type(ESMF_State), intent(in) :: state
...
! nested States
! \item[{[filename]}]
! The filename to which to write the Attribute info. This debugging option
! should not have been made public and will be removed in the future.
! \item[{[rc]}]
! Return code; equals {\tt ESMF\_SUCCESS} if there are no errors.
! \end{description}
!
!EOP
       ...
       call statePrintWorker (state%statep, level=0, filename=filename, rc1=rc)
   contains
     recursive subroutine statePrintWorker (sp1, level, filename, rc1)
       ...
       if (debugflag) then
         ...
         if (ESMF_LogFoundError(localrc1, &
           ESMF_ERR_PASSTHRU, &
           ESMF_CONTEXT, rcToReturn=rc1)) return
         call c_ESMC_BasePrint(sp1%base, level, "debug", ESMF_TRUE, filename, &
                               ESMF_FALSE, localrc1)
         if (ESMF_LogFoundError(localrc1, &
           ESMF_ERR_PASSTHRU, &
           ESMF_CONTEXT, rcToReturn=rc1)) return
       ...
       do, i1 = 1, ptrcnt
         ...
         call ESMF_StateItemPrint (sip, &
             header=trim (outbuf), &
             prefixstr=nestr, &
             longflag=longflag, debugflag=debugflag, filename=filename)
